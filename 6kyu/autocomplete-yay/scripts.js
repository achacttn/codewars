...
const dict = ['airplane', 'airport', 'apple', 'ball'];

function autocomplete(input, dictionary) {
    const letters = "qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM".split("");
    let originalLetters = input.split("").filter( eachLetter => letters.includes(eachLetter)).join("");
    // console.log('input: ', input, 'originalLetters: ', originalLetters);
    let strlen = originalLetters.length;
    let results = [];
    for( var i=0; i<dictionary.length; i++ ){
        if( dictionary[i].substring(0,strlen).toLowerCase() === originalLetters.toLowerCase() ){
            results.push(dictionary[i]);
            // results.push(`${originalLetters}${dictionary[i].substring(strlen)}`);
        }
    }
    return results.slice(0,5);
};

console.log(autocomplete('ai', dict));