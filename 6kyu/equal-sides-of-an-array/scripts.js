...
const findEvenIndex = arr => {
    const numArr = Array.prototype.slice.call(arr);
    ...
    for( let i=0, len=numArr.length; i<len; i++ ){
        let left = numArr.slice(0,i), right = numArr.slice(i+1);
        console.log(sumFunc(left), sumFunc(right));
        if( sumFunc(left)===sumFunc(right) ){
            return i;
        }
    }
    return -1;
